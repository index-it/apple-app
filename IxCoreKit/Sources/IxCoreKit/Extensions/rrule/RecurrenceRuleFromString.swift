//
//  index
//  RecurrenceRuleFromString.swift
//
...
struct RecurrenceParser {
    ...
    var interval: Int {
        let intervalArray = ruleString.components(separatedBy: "INTERVAL=")
    ...
    var type: EKRecurrenceFrequency? {
        if ruleString.contains("FREQ=DAILY") {
            ...
            return .monthly
        } else if ruleString.contains("FREQ=YEARLY") {
            return .yearly
        } else {
            return nil
        }
    ...
    var end: EKRecurrenceEnd? {
        let untilArray = ruleString.components(separatedBy: "UNTIL=")
    ...
    fileprivate func processRegexp(_ regexString: String) -> [NSString]? {
        ...
        if let match = reg?.firstMatch(in: String(ruleString),
                                       options: .reportProgress,
                                       range: NSRange(location: 0, length: ruleString.length))
        {
            return ruleString.substring(with: match.range(at: 1)).components(separatedBy: ",") as [NSString]?
        }
        return nil
    }
