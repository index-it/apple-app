    ...
    func parseRRule(_ rrule: String?) {
        if let rrule = rrule {
            ...
            if let rule = rule {
                ...
                self.recurrenceCount = rule.interval
                
                switch rule.frequency {
                case .daily:
                    self.recurrenceFrequency = .daily
                case .weekly:
                    self.recurrenceFrequency = .weekly
                ...
                case .monthly:
                    self.recurrenceFrequency = .monthly
                    
                    if rule.daysOfTheMonth != nil && !rule.daysOfTheMonth!.isEmpty {
                        self.monthFrequencies = Set(rule.daysOfTheMonth!.map { n in n.intValue })
                    } else {
                        self.monthEachSelected = false
                        if let dayOfWeek = rule.daysOfTheWeek?.first {
                            switch dayOfWeek.dayOfTheWeek {
                            case .sunday:
                                self.monthlyWeekdayFrequencyTarget = .sunday
                            case .monday:
                                self.monthlyWeekdayFrequencyTarget = .monday
                            case .tuesday:
                                self.monthlyWeekdayFrequencyTarget = .tuesday
                            case .wednesday:
                                self.monthlyWeekdayFrequencyTarget = .wednesday
                            case .thursday:
                                self.monthlyWeekdayFrequencyTarget = .thursday
                            case .friday:
                                self.monthlyWeekdayFrequencyTarget = .friday
                            case .saturday:
                                self.monthlyWeekdayFrequencyTarget = .saturday
                            }
                            
                            if dayOfWeek.weekNumber <= 1 {
                                self.monthlyWeekdayFrequency = .first
                            } else if dayOfWeek.weekNumber == 2 {
                                self.monthlyWeekdayFrequency = .second
                            } else if dayOfWeek.weekNumber == 3 {
                                self.monthlyWeekdayFrequency = .third
                            } else if dayOfWeek.weekNumber == 4 {
                                self.monthlyWeekdayFrequency = .fourth
                            } else if dayOfWeek.weekNumber == 5 {
                                self.monthlyWeekdayFrequency = .fifth
                            } else if dayOfWeek.weekNumber == 6 {
                                self.monthlyWeekdayFrequency = .last
                            }
                        }
                    }
                case .yearly:
                    self.recurrenceFrequency = .yearly
                    
                    if rule.monthsOfTheYear != nil && !rule.monthsOfTheYear!.isEmpty {
                        self.yearFrequencies = Set(rule.monthsOfTheYear!.map { n in n.intValue })
                    }
                    
                    if let dayOfWeek = rule.daysOfTheWeek?.first {
                        switch dayOfWeek.dayOfTheWeek {
                        case .sunday:
                            self.yearlyWeekdayFrequencyTarget = .sunday
                        case .monday:
                            self.yearlyWeekdayFrequencyTarget = .monday
                        case .tuesday:
                            self.yearlyWeekdayFrequencyTarget = .tuesday
                        case .wednesday:
                            self.yearlyWeekdayFrequencyTarget = .wednesday
                        case .thursday:
                            self.yearlyWeekdayFrequencyTarget = .thursday
                        case .friday:
                            self.yearlyWeekdayFrequencyTarget = .friday
                        case .saturday:
                            self.yearlyWeekdayFrequencyTarget = .saturday
                        }
                        
                        if dayOfWeek.weekNumber <= 1 {
                            self.yearlyWeekdayFrequency = .first
                        } else if dayOfWeek.weekNumber == 2 {
                            self.yearlyWeekdayFrequency = .second
                        } else if dayOfWeek.weekNumber == 3 {
                            self.yearlyWeekdayFrequency = .third
                        } else if dayOfWeek.weekNumber == 4 {
                            self.yearlyWeekdayFrequency = .fourth
                        } else if dayOfWeek.weekNumber == 5 {
                            self.yearlyWeekdayFrequency = .fifth
                        } else if dayOfWeek.weekNumber == 6 {
                            self.yearlyWeekdayFrequency = .last
                        }
                    }
                    
                    if let end = rule.recurrenceEnd {
                        if let endDate = end.endDate {
                            self.endRepeat = .onDate
                            self.endRepeatDate = endDate
                        } else {
                            self.endRepeatAfterOccurrences = end.occurrenceCount
                        }
                    }
                @unknown default:
